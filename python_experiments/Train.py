...
SizeX=X.shape[1]; SizeY=1; SizeZ=1 #Input img will be resized to this size
...
keep_prob = tf.placeholder(tf.float32) #dropout (keep probability -currently not used)
...
with tf.Session() as sess:
	...
	summary_op = tf.summary.merge_all()
        summary = sess.run([summary_op,optimizer], feed_dict={x: data, y: label})
