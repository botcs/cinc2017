...
flags.DEFINE_integer(
  'batch_size',
  def_batch_size,
  'Number of samples per update cycle [%d]' %
  def_batch_size)
flags.DEFINE_integer(
  'time_steps',
  def_time_steps,
  'Length of unrolled LSTM network [%d]' %
  def_time_steps)
flags.DEFINE_string(
  'rnn_sizes',
  '10, 10, 10',
  'Stacked RNN state size. Use comma separated integers ["10, 10, 10"]')
flags.DEFINE_string(
  'fc_sizes',
  '30, 10',
  'Size of fully connected layers. Use comma separated integers ["30, 10"]')
flags.DEFINE_float(
  'keep_prob',
  def_keep_prob,
  'Probability of keeping an activation value after the DROPOUT layer, during training [%f]' %
  def_keep_prob)
flags.DEFINE_string(
  'model_path',
  '/tmp/model',
  'Logs will be saved to this directory')
...
class stackedLSTM(object):
  ...
  def get_input(self, batch_size):
    with tf.variable_scope('input'):
      x = tf.placeholder(
        tf.float32, [
          batch_size, None, def_input_dim], name='X')
    ...
    with tf.variable_scope('LSTM'):
      ...
      cells = [
        tf.contrib.rnn.DropoutWrapper(
          cell, output_keep_prob=keep_prob) for cell in cells]
